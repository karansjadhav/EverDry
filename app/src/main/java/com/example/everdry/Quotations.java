...
public class Quotations extends AppCompatActivity {
    ...
    MyAdapterUserQuotation myAdapterUserQuotation;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recyclerView = findViewById(R.id.quotation_list);
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        list = new ArrayList<>();
        myAdapterUserQuotation = new MyAdapterUserQuotation(this,list);
        recyclerView.setAdapter(myAdapterUserQuotation);

        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for(DataSnapshot dataSnapshot : snapshot.child(uid).child("quotations").getChildren()) {
                    Quotation q = dataSnapshot.getValue(Quotation.class);
                    list.add(q);
                }
                myAdapterUserQuotation.notifyDataSetChanged();
            }
    ...
    public void onBackPressed() {
        Intent intent = new Intent(Quotations.this, MainActivity.class);
